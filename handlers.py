...
async def handler(data: types.CallbackQuery, bot: Bot):
    try:
                        ...
                        for url in tweet_info["medias"][0]["urls"]:
                            if url["resolution"] == res:
                                ...
                                if size >= 20971520:
                                    ...
                                    dl = False
                                try:
                                    await bot.send_video(message.chat.id, video, thumb=tweet_info["medias"][0]["thumbnail_url"], reply_to_message_id=message.reply_to_message.message_id, caption=caption)
                                    await message.delete()
                                except BadRequest:
                                    mention = f"<a href='tg://user?id={message.from_user.id}'>@{data.from_user.full_name}</a>"
                                    caption = f"{caption} \n {mention}"
                                    await bot.send_video(message.chat.id, video, thumb=tweet_info["medias"][0]["thumbnail_url"], caption=caption)
                                if dl:
                                    remove(video)
                                break
